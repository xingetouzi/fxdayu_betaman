...
# 最大回撤及恢复期
def drawdown_periods(returns, top=10):
    """
...
def max_median_position_concentration(positions):
    """
...
# 行业风险暴露（行业资金分配）
def sector_exposures(positions, symbol_sector_map):
    """
...
# 换手率
def turnover(transactions, positions):
...
# 注：1bps = 0.0001
# A股一个点是0.01 即100bps
def slippage_sweep(returns, transactions, positions,
...
# 滑点敏感度 —— 不同滑点下对应的年化收益率
def slippage_sensitivity(returns,
                         transactions,
                         positions,
...
# 返回指定资金量下的修正夏普率
def capacity_sweep(returns, transactions, market_data,
                   bt_starting_capital,
                   min_pv=100000,
                   max_pv=300000000,
                   step_size=1000000):
...
# 每支股票的收益贡献
def profit_attribution(round_trips):
    """
...
# 下盈利单的可能性分布
def prob_profit_trade(round_trips):
    """
