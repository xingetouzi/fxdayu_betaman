...
class Dimensions:
    ...
    def __init__(self, dv, signal_data, industry_standard="sw1"):
        self.signal_data = signal_data
        self.sw1 = dv.get_ts(industry_standard)
        self.cap = dv.get_ts("float_mv")  # TODO not sure about this indicator

        self.methods = {
            "mad": self._mad,
            "barra": partial(self._barra, cap_series=dv.get_ts("float_mv").stack()),
    ...
    def __call__(self, signal_data=None, regression_preprocessing=("mad",),
                 ...
                 rank_ic_preprocessing=("neutralization_both", "standard_scale"),
                 p_threshold=0.05, profit_signal_preprocessing=("standard_scale", )):
        ...
        coef_df = pfm.calc_ic_stats_table(result[["回归系数", "IC", "最大回报IC", "最低回报IC"]])
        ...
        try:
            signal_data["quantiles"] = signal_data["signal"].groupby(level=0).apply(pd.qcut, q=5, labels=[1,2,3,4,5])
        except ValueError:
        ...
        profit_df = self._profit_df(copy_signal_data)
        up_space_df = self._profit_df(copy_signal_data.drop("return", axis=1).rename({"upside_ret": "return"}, axis=1))
        down_space_df = self._profit_df(copy_signal_data.drop("return", axis=1).rename({"downside_ret": "return"}, axis=1))
    ...
    def _profit_df(self, signal_data):
        ...
        q_way = top_bottom_signal(signal_data, way="quantiles") if "quantiles" in signal_data.columns else None
        ...
        if q_way is not None:
            series4 = self.pairwise_describe_and_mean_difference(q_way, ["quantiles: ", "top", "bottom"])
        else:
            series4 = None
    ...
    def _cross_sectional_function(self, date, df):
        ...
        signal_series = df.pop("signal")
        ret = df.pop("return")
        upside_ret = df.pop("upside_ret")
        downside_ret = df.pop("downside_ret")
        ...
        return regression_results.tvalues[0], regression_results.params[0], regression_results.pvalues[0],\
               (*self.two_column_rank_ic(ret, signal_ic)), \
               (*self.two_column_rank_ic(upside_ret, signal_ic)), (*self.two_column_rank_ic(downside_ret, signal_ic))
...
class Evaluator:
    ...
    def __call__(self, period, comp=None,
        ...
        if industry is not None:
            ...
            industry_df = self.dv.get_ts(industry_standard).stack()
            data = data[industry_df.isin(industry)]
        ...
        return Dimensions(self.dv, data, industry_standard=industry_standard)
